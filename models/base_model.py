...
class BaseModel():
    ...
    def load_networks(self, epoch, save_dir=None, logger = None):
        if save_dir is None:
            save_dir = self.save_dir
        
        for name in self.model_names:
            if isinstance(name, str): # and name == 'D':
                ...
                if not os.path.exists(load_path):
                    print('model path: %s is not exists, attenion!!!'%load_path)
                    continue
                print('basemodel loading the model from %s'% load_path)
                # if you are using PyTorch newer than 0.4 (e.g., built from
                # GitHub source), you can remove str() on self.device
                state_dict = torch.load(load_path, map_location=str(self.device))
                if logger:
                    logger.info('load fusion net from: %s'%load_path)
